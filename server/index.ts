...
async function enrichWithOpenAI(fallback: AutomationRun): Promise<AutomationRun> {
  ...
  const prompt = [
    `You are an AI automation consultant demoing AI Appointment Scheduling Agent.`,
...
app.listen(port, () => {
  console.log(`AI Appointment Scheduling Agent listening on http://localhost:${port}`);
});
